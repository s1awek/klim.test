...
add_filter( 'post_content', function ( $content, $post_id, $context ) {
  if ( array_key_exists( 'edit_post_id', $_GET ) && intval( $_GET['edit_post_id'] ) == $post_id && current_user_can( 'edit_post', $post_id ) ) {
...
add_filter( 'get_post_metadata', function ( $value, $object_id, $meta_key, $single ) {
  static $in_cap_check = false;

  // Bail before the capability check unless a URL override for this exact
  // meta key is present. current_user_can() reads post meta itself (core
  // trash status, membership plugins), so checking it on every meta read
  // recurses until the PHP worker crashes.
  if ( $in_cap_check || ! $meta_key || ! array_key_exists( $meta_key, $_GET ) ) {
    return $value;
  }
  ...
  return $can_edit ? sanitize_text_field( $_GET[ $meta_key ] ) : $value;
}, 10, 4 );
