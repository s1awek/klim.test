...
{
    return new \OmnibusProVendor\WPDesk\Mutex\WordpressMySQLLockMutex($lockName, $waitForLockTimeout);
}
/**
...
{
    return \OmnibusProVendor\WPDesk\Mutex\WordpressMySQLLockMutex::fromOrder($order, $lockName, $waitForLockTimeout);
}
/**
 * Acquire lock.
 *
...
function wpdesk_acquire_lock($lockName, $waitForLockTimeout = 5, $lockType = 'mysql')
{
    if ('mysql' === $lockType) {
        ...
        $storage = new \OmnibusProVendor\WPDesk\Mutex\StaticMutexStorage();
...
function wpdesk_release_lock($lockNAme)
{
    ...
    if (null !== $mutex) {
        ...
        throw new \OmnibusProVendor\WPDesk\Mutex\MutexNotFoundInStorage();
    }
}
