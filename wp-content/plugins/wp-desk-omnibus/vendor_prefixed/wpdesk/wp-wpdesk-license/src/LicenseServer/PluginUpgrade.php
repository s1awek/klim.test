...
class PluginUpgrade
{
    ...
    {
        global $wp_version;
        $params = ['site_url' => defined('OmnibusProVendor\WP_HOME') ? \OmnibusProVendor\WP_HOME : get_home_url(), 'plugin_name' => $this->plugin_info->get_plugin_name(), 'plugin_slug' => $this->plugin_info->get_plugin_slug(), 'plugin_version' => $this->plugin_info->get_version(), 'php_version' => \PHP_VERSION, 'wc_version' => class_exists('WooCommerce') ? \WooCommerce::instance()->version : '', 'wp_version' => $wp_version, 'locale' => str_replace('-', '_', get_bloginfo('language'))];
    ...
    {
        $parser = new \OmnibusProVendor\WPDesk\License\Changelog\Parser($changelog);
        $parser->parse();
        $parsed_changelog = $parser->get_parsed_changelog()->getIterator();
        $changes = new \OmnibusProVendor\WPDesk\License\Changelog\Filter\ByVersion($parsed_changelog, $plugin_version);
        if (iterator_count($changes) > 0) {
            $changelogFormatter = new \OmnibusProVendor\WPDesk\License\Changelog\Formatter($changes);
