...
use OmnibusProVendor\DI\Annotation\Inject;
use OmnibusProVendor\DI\Annotation\Injectable;
use OmnibusProVendor\DI\Definition\Exception\InvalidAnnotation;
use OmnibusProVendor\DI\Definition\ObjectDefinition;
use OmnibusProVendor\DI\Definition\ObjectDefinition\MethodInjection;
use OmnibusProVendor\DI\Definition\ObjectDefinition\PropertyInjection;
use OmnibusProVendor\DI\Definition\Reference;
use OmnibusProVendor\Doctrine\Common\Annotations\AnnotationRegistry;
use OmnibusProVendor\Doctrine\Common\Annotations\Reader;
use OmnibusProVendor\Doctrine\Common\Annotations\SimpleAnnotationReader;
use InvalidArgumentException;
use OmnibusProVendor\PhpDocReader\PhpDocReader;
use ReflectionClass;
...
class AnnotationBasedAutowiring implements DefinitionSource, Autowiring
{
    ...
    private function readProperty(ReflectionProperty $property, ObjectDefinition $definition, $classname = null)
    {
        ...
        $annotation = $this->getAnnotationReader()->getPropertyAnnotation($property, 'OmnibusProVendor\DI\Annotation\Inject');
    ...
    private function getMethodInjection(ReflectionMethod $method)
    {
        // Look for @Inject annotation
        try {
            $annotation = $this->getAnnotationReader()->getMethodAnnotation($method, 'OmnibusProVendor\DI\Annotation\Inject');
    ...
    public function getAnnotationReader()
    {
        if ($this->annotationReader === null) {
            ...
            $this->annotationReader->addNamespace('OmnibusProVendor\DI\Annotation');
        }
    ...
    private function readInjectableAnnotation(ReflectionClass $class, ObjectDefinition $definition)
    {
        try {
            ...
            $annotation = $this->getAnnotationReader()->getClassAnnotation($class, 'OmnibusProVendor\DI\Annotation\Injectable');
