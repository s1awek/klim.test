...
<div class="wpallimport-collapsed wpallimport-section">
	...
	<div class="wpallimport-content-section">
		...
		if ( "new" == $post['wizard_type']): 
			...
			{
				?>
				<h3><?php _e('Record Matching', 'wpai_woocommerce_addon_plugin'); ?></h3>
				<?php 
			}
			else
			{ 					
				...
				{
					?>
					<h3 style="padding-left:0;"><?php _e('Choose how exported data will be re-imported.','wpai_woocommerce_addon_plugin');?></h3>
					<?php 
				}
				else
				{
					?>
					<h3 style="padding-left:0;"><?php printf(__('WP All Import will create new %s for each unique record in your file.','wpai_woocommerce_addon_plugin'), $custom_type->labels->name);?></h3>
					<?php 
				}
			}
					...
					<table class="form-table" style="max-width:none;">
						<tr>
							<td>
								...
								<?php if ( ! $isWizard ):?>												 
								<h4><?php printf(__('WP All Import will associate records in your file with %s it has already created from previous runs of this import based on the Unique Identifier.','wpai_woocommerce_addon_plugin'), $custom_type->labels->name);?></h4>
								<?php endif; ?>
		...
		<?php else: ?>
			<?php if ( ! $isWizard ):?>
				<h3><?php _e('Record Matching', 'wpai_woocommerce_addon_plugin'); ?></h3>
			<?php else: ?>
				<h3 style="padding-left:0;"><?php printf(__('WP All Import will merge data into existing %s.','wpai_woocommerce_addon_plugin'), $custom_type->labels->name);?></h3>
			<?php endif; ?>
			</div>
			<div class="wpallimport-collapsed-content" style="padding:0;">				
				<div class="wpallimport-collapsed-content-inner">					
						...
						<tr>
							<td>						
								<div class="input" style="margin-bottom:15px; position:relative;">					
									...
									<h4><?php printf(__('Records in your file will be matched with %ss on your site based on...', 'wpai_woocommerce_addon_plugin' ), $custom_type->labels->singular_name);?></h4>
