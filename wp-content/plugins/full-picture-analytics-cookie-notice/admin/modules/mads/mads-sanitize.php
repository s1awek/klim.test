	...
	if( ! empty( $value ) ) {
		...
		switch ($clean_key) {
			...
			case 'track_file_downl':
				...
				if ( is_array( $value ) && ! empty( $value['formats'] ) && ! empty( $value['val'] ) ) {
					$clean_val['formats'] = trim( sanitize_text_field( $value['formats'] ) );
					$clean_val['val'] = trim( sanitize_text_field( $value['val'] ) );
				};

				break;

			case 'id':
			case 'woo_action_name':
