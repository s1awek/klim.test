		...
		$catalog = [
			'google_ads'    => [
				...
				'settings' => [
					...
					'data_manager.is_active'                 => [
						'path'        => 'google.ads.data_manager.is_active',
						'type'        => 'boolean',
						'label'       => 'Data Manager API upload',
						'description' => 'Experimental: uploads purchase conversions to Google Ads through the Data Manager API, in addition to the browser tag. Deduplicated by transaction ID.',
						'advanced'    => true,
						'benefit'     => 'Recovers conversions the browser tag misses, for example when payment gateways skip the confirmation page or ad blockers stop the tag.',
					],
					'data_manager.mode'                      => [
						'path'        => 'google.ads.data_manager.mode',
						'type'        => 'string',
						'label'       => 'Data Manager upload mode',
						'description' => 'Experimental. "multi_source" uploads into the same conversion action as the browser tag (deduplicated by transaction ID, recommended). "separate_action" uploads into a dedicated import conversion action, which should be set to Secondary in Google Ads to avoid double counting.',
						'enum'        => [ 'multi_source', 'separate_action' ],
						'advanced'    => true,
						'benefit'     => 'Controls whether API conversions merge with the tag conversions or are counted independently for comparison.',
					],
					'data_manager.operating_account_id'      => [
						'path'        => 'google.ads.data_manager.operating_account_id',
						'type'        => 'string',
						'label'       => 'Data Manager operating account ID',
						'description' => 'Experimental. The Google Ads customer ID that owns the conversion action, 8 to 12 digits. Dashes are stripped automatically.',
						'advanced'    => true,
					...
					'data_manager.login_account_id'          => [
						...
						'type'        => 'string',
						'label'       => 'Data Manager login account ID',
						'description' => 'Experimental. Optional manager (MCC) account customer ID used for access, 8 to 12 digits. Leave empty when the service account was added directly to the operating account.',
						'advanced'    => true,
					...
					'data_manager.conversion_action_id'      => [
						...
						'type'        => 'string',
						'label'       => 'Data Manager conversion action ID',
						'description' => 'Experimental. The numeric ID of the Google Ads conversion action that receives the uploads. Visible as "ctId" in the URL when the conversion action is opened in Google Ads.',
						'advanced'    => true,
						'benefit'     => 'Targets the uploads at the correct conversion action.',
					],
					'data_manager.auth_method'               => [
						'path'        => 'google.ads.data_manager.auth_method',
						'type'        => 'string',
						'label'       => 'Data Manager authentication method',
						'description' => 'Experimental. "service_account" authenticates with a Google Cloud service account key that was added to the Google Ads account. "broker" routes uploads through the SweetCode connect service.',
						'enum'        => [ 'service_account', 'broker' ],
						'advanced'    => true,
						'benefit'     => 'Selects how the plugin authenticates against the Data Manager API.',
					],
					'data_manager.validate_only'             => [
						'path'        => 'google.ads.data_manager.validate_only',
						'type'        => 'boolean',
						'label'       => 'Data Manager test mode',
						'description' => 'Experimental. When enabled, uploads are sent with validateOnly so Google checks them without recording conversions.',
						'advanced'    => true,
						'benefit'     => 'Verifies the setup end to end before real conversions are uploaded.',
					],
				],
			],
			'ga4'           => [
