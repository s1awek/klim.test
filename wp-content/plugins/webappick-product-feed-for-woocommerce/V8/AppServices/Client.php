...
class Client {
	...
	/*
	 * The upstream SDK also ships license() and updater() accessors. CTX Feed
	 * handles licensing through its own Pro license SDK and WordPress.org
	 * updates, so both accessors are deliberately absent here; see the file
	 * history for the original bodies.
	 */
	...
	private function endpoint( $route = '' ) {
		 ...
		 * @param array     $params
		 */
		// phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.DynamicHooknameFound -- Hook name is prefixed at runtime with $this->slug (the plugin folder slug); it is part of the published WebAppick services SDK contract and cannot be renamed.
		 ...
		 * @param string $url
		 */
		// phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.DynamicHooknameFound, WordPress.NamingConventions.ValidHookName.NotLowercase -- Hook name is prefixed at runtime with $this->slug; the mixed-case suffix is the published WebAppick services SDK contract and renaming it would silently drop existing integrations.
	...
	protected function set_basename_and_slug() {
		...
		if ( false === strpos( $this->file, WP_CONTENT_DIR . '/themes/' ) ) {
			...
			list( $this->slug, $mainfile ) = explode( '/', $this->basename ); // phpcs:ignore VariableAnalysis.CodeAnalysis.VariableAnalysis.UnusedVariable -- $mainfile is a positional placeholder in the list() destructuring; only the leading slug is needed.
			if ( ! function_exists( 'get_plugin_data' ) ) {
				require_once ABSPATH . 'wp-admin/includes/plugin.php';
			}
			// The free plugin's main file is used deliberately so Pro reports the free version.
			...
			list( $this->slug, $main_file ) = explode( '/', $this->basename ); // phpcs:ignore VariableAnalysis.CodeAnalysis.VariableAnalysis.UnusedVariable -- $main_file is a positional placeholder in the list() destructuring; only the leading slug is needed.
	 ...
	 * @return string
	 */
	private function __user_agent() { // phpcs:ignore PHPCompatibility.FunctionNameRestrictions.ReservedFunctionNames.MethodDoubleUnderscore -- Method name is part of the vendored WebAppick services SDK surface; renaming it would diverge from upstream. It is private, so it can never collide with a future PHP magic method dispatch.
	...
	public function send_request( $params, $route = '', $blocking = false ) {
		...
		$response = wp_safe_remote_post(
			...
			array(
				'method'      => 'POST',
				'timeout'     => 45,   // phpcs:ignore WordPressVIPMinimum.Performance.RemoteRequestTimeout.timeout_timeout -- Telemetry/licence calls to track.webappick.com need the full budget; the request is non-blocking by default, so the timeout never delays a page render.
				'redirection' => 5,
				'httpversion' => '1.0',
				'blocking'    => $blocking,
				'headers'     => $headers,
