<?php
/**
 * Aggregate compatibility shim for the dynamic-pricing / discount plugins.
 *
 * Each supported plugin applies its discount only in cart or front-end
 * context, so this class re-runs the relevant plugin's calculation while a
 * feed is being generated and hands the adjusted price back to the
 * `woo_feed_filter_product_*` chain.
 *
...
// phpcs:ignore Squiz.PHP.CommentedOutCode.Found, Squiz.Commenting.InlineComment.InvalidEndChar -- Kept from the V5 original: AWDP_Discount is imported by AcoWooDynamicPricing instead, and the disabled import records that this file deliberately does not depend on the class.
// use AWDP_Discount;
...
class DynamicDiscount {
	...
	/**
	 * DynamicDiscount Constructor.
	 */
	public function __construct() {
		// Discounted price filter.
		add_filter( 'woo_feed_filter_product_sale_price', array( $this, 'get_dynamic_discounted_product_price' ), 9, 5 );
		add_filter( 'woo_feed_filter_product_sale_price_with_tax', array( $this, 'get_dynamic_discounted_product_price' ), 9, 5 );

		add_filter( 'woo_feed_filter_product_price', array( $this, 'get_dynamic_discounted_product_price' ), 9, 5 );
		add_filter( 'woo_feed_filter_product_price_with_tax', array( $this, 'get_dynamic_discounted_product_price' ), 9, 5 );
	}
	 ...
	 * @param mixed                          $feedConfig Feed configuration object.
	...
	public function get_dynamic_discounted_product_price( $price, $product, $feedConfig, $tax, $price_type ) {
		...
		if ( is_plugin_active( 'easy-woocommerce-discounts/easy-woocommerce-discounts.php' ) ) {
			...
			if ( doing_action( 'woo_feed_update' ) || doing_action( 'woo_feed_update_single_feed' ) ) {
				...
				// phpcs:ignore Squiz.PHP.CommentedOutCode.Found, Squiz.Commenting.InlineComment.InvalidEndChar -- V5 note kept verbatim: records the two rule shapes ("all_products", "products_in_list") the adapter below has to cope with.
			...
			// phpcs:ignore Squiz.PHP.CommentedOutCode.Found -- Kept from the V5 original: documents the direct WCCS_Public_Product_Pricing path that was rejected in favour of the EasyWoocommerceDiscounts shim used above.
		...
		// Parentheses below are explicit only; `&&` already binds tighter
		// than `||`, so the evaluated result is unchanged.
